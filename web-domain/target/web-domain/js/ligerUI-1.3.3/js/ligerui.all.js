...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        {
            if (column.isHideColumn) return 0;
            var g = this, p = this.options;
            var columnLength = g.columns.length;
            if(p.hasAllRowColumn){
            ...
            if (typeof (colwidth) == "string" && colwidth.indexOf('%') > 0)
            {
                ...
                column._width = colwidth = parseInt(parseInt(colwidth) * 0.01 * (g.grid.width() - lwidth - (columnLength / 2) - 1));
            }
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                var rowid = item['__id'];
                if (!item) continue;
                var allRowData = '',allRowNotData = '', addrow = true;
                gridhtmlarr.push('<tr');
                ...
                {   if(column.showAllRow) {allRowData = g._getAllRowHtml(item, column);allRowNotData = g._getAllRowNotValueHtml(item, column);  return;}
                ...
                gridhtmlarr.push('</tr>');
                if(p.hasAllRowColumn){
                    var col = g.columns.length;
                    gridhtmlarr.push('<tr');
                    gridhtmlarr.push(' id="' + g._getRowDomId(item, frozen) + '_allrow"');
                    ...
                    gridhtmlarr.push('"');
                    if(addrow) {
                        gridhtmlarr.push(' colspan = "'+(col-1) +'">');
                        gridhtmlarr.push(allRowData);
                    }else{
                        gridhtmlarr.push('>');
                        gridhtmlarr.push(allRowNotData);
                    }
                    gridhtmlarr.push('</td>');
                    gridhtmlarr.push('</tr>');
                }
        ...
        _getAllRowHtml: function (rowdata, column)
        {
            var g = this, p = this.options;
            if (column.isrownumber)
                return '<div class="l-grid-row-cell-inner">' + (parseInt(rowdata['__index']) + 1) + '</div>';
            ...
            htmlarr.push('<div class="l-grid-row-cell-inner"');
            //htmlarr.push('<div');
            htmlarr.push(' style = "color: #147CAF;');
            htmlarr.push('min-height:' + p.rowHeight + 'px; ');
            if (column.align) htmlarr.push('text-align:' + column.align + ';');
            var name = '<span style="font-weight: bold;">'+column.display+'</span>';
            var content = g._getCellContent(rowdata, column);
            htmlarr.push('">' + name + " : " + content + '</div>');
            return htmlarr.join('');
        },
        _getAllRowNotValueHtml: function (rowdata, column)
        {
            var g = this, p = this.options;
            var htmlarr = [];
            htmlarr.push('<div class="l-grid-row-cell-inner"');
